...
async function processQueueRow(id) {
  ...
  try {
    await withTransaction(async (txQuery) => {
      ...
      await txQuery(
        `UPDATE channel_incoming_queue SET state = 'processing', processing_started_at = NOW(), last_attempt_at = NOW() WHERE id = ?`,
        [item.id],
      );
      ...
      const [settingRow] = await txQuery(
      ...
      if (credRow?.credentials) {
        let credStr =
          typeof credRow.credentials === 'string'
            ? credRow.credentials
            : JSON.stringify(credRow.credentials);
        try {
          const { decrypt } = require('./encryption');
          creds = JSON.parse(decrypt(credStr));
        } catch (e) {
          console.error('Failed to decrypt credentials for incoming webhook:', e);
        }
      }
      ...
      const providerMsgId =
        normalizedMsg.metadata?.message_id ||
        normalizedMsg.metadata?.messages?.[0]?.id ||
        normalizedMsg.metadata?.entry?.[0]?.messaging?.[0]?.message?.mid ||
        item.provider_message_id ||
        'incoming-mock-id-' + item.id;

      try {
        await txQuery(`INSERT INTO webhook_idempotency (provider_message_id) VALUES (?)`, [
          providerMsgId,
        ]);
      } catch (dbErr) {
        if (dbErr.code === '23505' || dbErr.message.includes('unique')) {
          ...
          await txQuery(
            `UPDATE channel_incoming_queue SET state = 'processed', last_error = 'Duplicate webhook message skipped', provider_message_id = ? WHERE id = ?`,
      ...
      eventBus.emit('incoming_message', {
        ...normalizedMsg,
        uid: item.uid,
        correlation_id: item.correlation_id,
      });
      ...
      await txQuery(
        `UPDATE channel_incoming_queue SET state = 'processed', provider_message_id = ? WHERE id = ?`,
        [providerMsgId, item.id],
      );
    });
  } catch (err) {
