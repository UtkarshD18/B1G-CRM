...
async function testAIProviderConnection(provider, model, apiKey, prompt, customEndpoint) {
  ...
  try {
    if (provider === "openai" || provider === "openrouter" || provider === "groq" || provider === "mistral" || provider === "deepseek" || provider === "custom" || provider === "ollama") {
      ...
      if (provider === "custom") baseURL = customEndpoint || (model && typeof model === "object" ? model.customBaseUrl : "") || "";
      if (provider === "ollama") baseURL = customEndpoint || "http://localhost:11434/v1";

      let url = `${baseURL}/chat/completions`;
      if ((provider === "custom" || provider === "deepseek" || provider === "ollama") && customEndpoint) {
        url = customEndpoint.endsWith("/chat/completions") ? customEndpoint : `${customEndpoint}/chat/completions`;
      }
...
async function executeAIProvider(provider, model, apiKey, systemPrompt, messages, temperature, maxTokens, customEndpoint) {
  const startTime = Date.now();
  let responseText = "";
  let tokensUsed = 0;
