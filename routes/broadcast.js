...
function parseDateBoundary(value, endOfDay = false) {
  ...
  const date = new Date(
    isDateOnly ? `${input}T${endOfDay ? '23:59:59.999' : '00:00:00.000'}Z` : input,
  );
...
router.post(
  '/add_new',
  validateUserOrAgent,
  verifyPermission('campaigns_access'),
  checkPlan,
  async (req, res) => {
    try {
      ...
      const getPhonebook = await query(`SELECT * FROM phonebook WHERE id = ? AND uid = ?`, [
        phonebook.id,
        req.decode.uid,
      ]);
        ...
        if (credRow?.credentials) {
          ...
          try {
            ...
            if (creds.access_token && creds.phone_number_id) {
              ...
              getMetaAPI = await query(`SELECT * FROM meta_api WHERE uid = ?`, [req.decode.uid]);
            }
          } catch (err) {
            console.error('Failed to decrypt credentials for campaign:', err);
          }
        }
      }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      appendDateFilters(campaignWhere, campaignParams, campaignDateExpression, range);
      ...
      const [campaigns, logs] = await Promise.all([
        ...
        query(`SELECT * FROM broadcast_log WHERE ${logWhere.join(' AND ')}`, logParams),
      ]);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await query(`UPDATE broadcast SET status = ? WHERE broadcast_id = ? AND uid = ?`, [
        status,
        broadcast_id,
        req.decode.uid,
      ]);
  ...
  async (req, res) => {
    try {
      ...
      await withTransaction(async (tx) => {
        ...
        await tx(`DELETE FROM broadcast_log WHERE uid = ? AND broadcast_id = ?`, [
          req.decode.uid,
          broadcast_id,
        ]);
      });
