...
function getCurrentTimestampInTimeZone(timezone) {
  return Math.round(moment().tz(timezone || "Asia/Kolkata").valueOf() / 1000);
}

// 1. Get Instagram Business Account Credentials
router.get("/get_keys", validateUserOrAgent, verifyPermission("settings_access"), async (req, res) => {
  try {
    const [keys] = await query("SELECT * FROM instagram_api WHERE uid = ?", [
      req.decode.uid,
    ]);
...
router.post("/save_keys", validateUserOrAgent, verifyPermission("settings_access"), async (req, res) => {
  try {
    ...
    const [existing] = await query("SELECT * FROM instagram_api WHERE uid = ?", [
      req.decode.uid,
    ]);
    ...
    if (existing) {
      await query(
        `UPDATE instagram_api 
         ...
         WHERE uid = ?`,
        [instagram_business_account_id, access_token, username, name || username, app_id || "", req.decode.uid]
      );
    } else {
      await query(
        `INSERT INTO instagram_api (uid, instagram_business_account_id, access_token, username, name, app_id) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [req.decode.uid, instagram_business_account_id, access_token, username, name || username, app_id || ""]
      );
    }
...
router.post("/disconnect", validateUserOrAgent, verifyPermission("settings_access"), async (req, res) => {
  try {
    ...
    if (existingChat) {
      ...
      await query(
        `INSERT INTO chats (chat_id, uid, last_message_came, sender_name, sender_mobile, last_message, is_opened, origin, profile) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [senderId, uid, userTimezone, senderName, senderId, JSON.stringify(newMessage), 0, "instagram", profilePic]
      );
    }
