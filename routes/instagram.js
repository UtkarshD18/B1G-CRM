...
router.get("/get_keys", validateUser, async (req, res) => {
  try {
...
router.post("/save_keys", validateUser, async (req, res) => {
  try {
...
router.post("/webhook/:uid", async (req, res) => {
  try {
    ...
    const appSecret = process.env.INSTAGRAM_APP_SECRET || "mock-app-secret";
    if (signature) {
