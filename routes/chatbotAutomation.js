  ...
  try {
    ...
    for (const flow of flows) {
      ...
      if (latestVer && latestVer.flow_json) {
        try {
          const flowJson = typeof latestVer.flow_json === 'string' ? JSON.parse(latestVer.flow_json) : latestVer.flow_json;
...
router.get("/flows/templates", validateUserOrAgent, verifyPermission("automation.template"), async (req, res) => {
  try {
...
router.get("/flows/:flowId", validateUserOrAgent, verifyPermission("automation.read"), async (req, res) => {
  try {
    ...
    if (flow.locked_by) {
      const lockExpired = new Date(flow.locked_at).getTime() + lockTimeout * 60 * 1000 < Date.now();
      if (!lockExpired) {
...
router.post("/flows", validateUserOrAgent, verifyPermission("automation.edit"), checkPlan, async (req, res) => {
  try {
      ...
      if (latestVer && latestVer.status === 'draft') {
        ...
        await tx(
          `UPDATE automation_flow_versions 
           ...
           WHERE id = ?`,
          [JSON.stringify(flowJson), name, versionNotes || latestVer.version_notes, checksum, newRevision, latestVer.id]
        );
      } else {
        ...
        await tx(
          ...
          [
            uid, flowId, nextVersionNum, name, JSON.stringify(flowJson), userId,
            versionNotes || 'Draft auto-save', checksum, releaseTag || 'Draft', newRevision
          ]
        );
      }
      ...
      if (!flow || flow.is_published === 0) {
        ...
        for (const n of (nodes || [])) {
          ...
          await tx(
            `INSERT INTO automation_nodes (flow_id, node_id, type, position_x, position_y, data) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            [flowId, n.id, n.type || "Send Message", n.position?.x || 0, n.position?.y || 0, JSON.stringify(nodeDataToSave)]
          );
        }
        ...
        for (const e of (edges || [])) {
          await tx(
            `INSERT INTO automation_edges (flow_id, edge_id, source, target, source_handle, target_handle) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            [flowId, e.id || `edge-${e.source}-${e.target}`, e.source, e.target, e.sourceHandle || null, e.targetHandle || null]
          );
        }
      }
    });
    ...
    res.json({ success: true, revision: flow ? flow.revision + 1 : 1, msg: "Flow saved successfully" });
  } catch (err) {
...
router.post("/flows/unlock", validateUserOrAgent, verifyPermission("automation.edit"), async (req, res) => {
  try {
...
router.post("/flows/publish", validateUserOrAgent, verifyPermission("automation.publish"), async (req, res) => {
  try {
    ...
    if (!latestVer) {
      return res.json({ success: false, msg: "No versions found for this flow. Please save first." });
    }
    ...
    if (currPublished && currPublished.checksum === latestVer.checksum) {
      ...
      await logActivity(req, "Automation", "flow_publish_reused", flowId, { version: currPublished.version });
      return res.json({
        success: true,
      ...
      if (latestVer.status === 'draft') {
        ...
        await tx(
          `UPDATE automation_flow_versions 
           ...
           WHERE id = ?`,
          [userId, releaseTag || 'Production', versionNotes || latestVer.version_notes, latestVer.id]
        );
      } else {
        ...
        await tx(
          ...
          [
            uid, flowId, nextVersionNum, latestVer.name, JSON.stringify(flowJson),
            latestVer.created_by, userId, versionNotes || 'Published release',
            latestVer.checksum, releaseTag || 'Production'
          ]
        );
      }
...
router.post("/flows/rollback", validateUserOrAgent, verifyPermission("automation.rollback"), async (req, res) => {
  try {
    ...
    const flowJson = typeof targetVer.flow_json === 'string' ? JSON.parse(targetVer.flow_json) : targetVer.flow_json;
    ...
    await withTransaction(async (tx) => {
      ...
      await tx(
        ...
        [
          uid, flowId, nextVersion, targetVer.name, JSON.stringify(flowJson),
          targetVer.created_by, userId, version,
          versionNotes || `Rollback to version ${version}`, targetVer.checksum
        ]
      );
    ...
    await logActivity(req, "Automation", "flow_rollback", flowId, { fromVersion: version, toVersion: nextVersion });
...
router.post("/flows/validate", validateUserOrAgent, verifyPermission("automation.edit"), async (req, res) => {
  try {
...
router.get("/flows/:flowId/compare", validateUserOrAgent, verifyPermission("automation.compare"), async (req, res) => {
  try {
    const { flowId } = req.params;
    ...
    if (!v1 || !v2) {
      return res.status(400).json({ success: false, msg: "v1 and v2 query parameters are required" });
    }
    ...
    if (!ver1 || !ver2) {
      return res.status(404).json({ success: false, msg: "One or both specified versions could not be found" });
    }
...
router.get("/flows/:flowId/versions", validateUserOrAgent, verifyPermission("automation.history"), async (req, res) => {
  try {
    const { flowId } = req.params;
    const uid = req.decode.uid;
...
router.get("/flows/:flowId/versions/:version", validateUserOrAgent, verifyPermission("automation.history"), async (req, res) => {
  try {
    ...
    const flowJson = typeof ver.flow_json === 'string' ? JSON.parse(ver.flow_json) : ver.flow_json;
    const nodes = flowJson.nodes || [];
    const edges = flowJson.edges || [];
...
router.get("/flows/:flowId/versions/:version/export", validateUserOrAgent, verifyPermission("automation.export"), async (req, res) => {
  try {
    const { flowId, version } = req.params;
    const uid = req.decode.uid;
    ...
    res.setHeader("Content-Disposition", `attachment; filename="flow_${flowId}_v${version}.json"`);
    res.json({
      flowId: ver.flow_id,
      version: ver.version,
      name: ver.name,
...
router.post("/flows/import", validateUserOrAgent, verifyPermission("automation.import"), async (req, res) => {
  try {
    ...
    if (!flowId || !name || !flow_json) {
      return res.status(400).json({ success: false, msg: "flowId, name, and flow_json are required" });
    }
    ...
    await withTransaction(async (tx) => {
      ...
      const [existing] = await tx(`SELECT * FROM automation_flows WHERE flow_id = ? AND uid = ?`, [flowId, uid]);
      let revision = 1;
      ...
      const [latest] = await tx(`SELECT version FROM automation_flow_versions WHERE flow_id = ? AND uid = ? ORDER BY version DESC LIMIT 1`, [flowId, uid]);
      ...
      await tx(
        ...
        [uid, flowId, nextVer, name, JSON.stringify(flow_json), userId, versionNotes || 'Imported flow schema', checksum, revision]
      );
    });
...
router.post("/flows/:flowId/template", validateUserOrAgent, verifyPermission("automation.template"), async (req, res) => {
  try {
    const { flowId } = req.params;
...
router.post("/flows/clone-template", validateUserOrAgent, verifyPermission("automation.template"), async (req, res) => {
  try {
    ...
    const checksum = crypto.createHash("sha256").update(JSON.stringify(template.flow_json)).digest("hex");
...
router.post("/flows/delete", validateUserOrAgent, verifyPermission("automation.edit"), async (req, res) => {
  try {
    ...
    await withTransaction(async (tx) => {
      ...
      await tx(`DELETE FROM automation_flow_versions WHERE flow_id = ? AND uid = ?`, [flowId, uid]);
...
router.post("/flows/duplicate", validateUserOrAgent, verifyPermission("automation.edit"), async (req, res) => {
  try {
    ...
    if (latestVer) {
      flowJson = typeof latestVer.flow_json === 'string' ? JSON.parse(latestVer.flow_json) : latestVer.flow_json;
    } else {
