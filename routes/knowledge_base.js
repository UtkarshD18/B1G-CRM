...
router.post("/reindex/:id", validateUserOrAgent, verifyPermission("kb.reindex"), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get("/chunks/:id", validateUserOrAgent, verifyPermission("ai.chunks"), async (req, res) => {
  try {
    ...
    const kbDoc = await query("SELECT id, title FROM knowledge_base WHERE id = ? AND uid = ?", [id, req.decode.uid]);
    if (kbDoc.length === 0) {
...
router.delete("/delete/:id", validateUserOrAgent, verifyPermission("kb.delete"), async (req, res) => {
  try {
    const { id } = req.params;
    const result = await query(
