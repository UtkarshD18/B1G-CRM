...
router.post("/leads/add", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
...
router.post("/leads/move", validateUserOrAgent, verifyPermission("kanban_access"), async (req, res) => {
  try {
...
router.post("/leads/update", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
...
router.post("/leads/delete", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.json({ success: false, msg: "Lead ID is required" });
...
router.get("/leads/reminders/:leadId", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
...
router.post("/leads/add_reminder", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
...
router.get("/leads/activities/:leadId", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
    const leadCheck = await query("SELECT id FROM crm_leads WHERE id = ? AND uid = ?", [req.params.leadId, req.decode.uid]);
    if (leadCheck.length === 0) {
      return res.json({ success: false, msg: "Lead not found or unauthorized" });
...
router.post("/leads/add_activity", validateUserOrAgent, verifyPermission("leads_access"), async (req, res) => {
  try {
...
router.post("/leads/update_pipeline_order", validateUserOrAgent, verifyPermission("kanban_access"), async (req, res) => {
  try {
