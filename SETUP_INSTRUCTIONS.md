...
B1G-CRM/
├── database/
│   ├── schema.sql          # demo-live reference schema
│   ├── postgres-local-schema.sql # local PostgreSQL compatibility schema
...
**Solution**: Change PORT in `.env` or kill process: `lsof -i :3010` → `kill -9 <PID>`
