...
*   **Created [nodemon.json](nodemon.json)**: Prevents dev server restarts when writing visual flows, inbox logs, or session credentials.
*   **Enabled Mock Mode in [.env](.env)**: Set `MOCK_META_DELIVERY=true` to bypass live Meta Graph API calls.
