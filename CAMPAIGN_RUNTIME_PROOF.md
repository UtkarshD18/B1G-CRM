...
*   **Evidence**: 
    - Calling `POST /api/broadcast/add_new` without credentials in the `meta_api` table returns:
      ...
      ```
    - Path: [routes/broadcast.js](routes/broadcast.js#L265-L274)
      ```javascript
...
*   **Evidence**:
    - When mock/dummy credentials (`access_token = 'example-token'`, `business_phone_number_id = 'mock_phone_id_123'`) are present in `meta_api`, the backend attempts to query the Graph API `v18.0` via `getMetaNumberDetail()`.
    - Because the mock credentials fail authentication with Meta, the endpoint returns:
      ...
      ```
    - Path: [routes/broadcast.js](routes/broadcast.js#L288-L299)
      ```javascript
...
*   **Loop Worker Location**: [loops/campaignLoop.js](loops/campaignLoop.js) (runs in background to pull campaigns in `'QUEUE'` status).
