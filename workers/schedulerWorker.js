...
async function executeRetentionPolicies(config) {
  try {
    ...
    await query(`DELETE FROM channel_outgoing_queue WHERE created_at < NOW() - INTERVAL '${outgoingQueueRetentionDays} days' AND state IN ('sent', 'failed', 'dead_letter')`);
    await query(`DELETE FROM channel_incoming_queue WHERE created_at < NOW() - INTERVAL '${outgoingQueueRetentionDays} days' AND state IN ('processed', 'failed')`);
    await query(`DELETE FROM channel_dead_letter_queue WHERE created_at < NOW() - INTERVAL '${deadLetterRetentionDays} days'`);
    await query(`DELETE FROM channel_metrics WHERE updated_at < NOW() - INTERVAL '${metricsRetentionDays} days'`);
    await query(`DELETE FROM webhook_idempotency WHERE processed_at < NOW() - INTERVAL '${webhookIdempotencyRetentionDays} days'`);
    await query(`DELETE FROM transport_workers WHERE last_seen < NOW() - INTERVAL '${workerHeartbeatStaleMinutes} minutes'`);
