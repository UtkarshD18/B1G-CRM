...
(async () => {
  ...
  try {
    ...
    const createRes = await axios.post(
      'http://localhost:3010/api/user/add_widget',
      {
        title: widgetTitle,
    ...
    console.log(
      `Fetching public widget rendering endpoint: GET /api/user/widget?id=${widgetUniqueId}...`,
    );
    const publicRenderRes = await axios.get(
      `http://localhost:3010/api/user/widget?id=${widgetUniqueId}`,
    );
    ...
    const deleteRes = await axios.post(
      'http://localhost:3010/api/user/del_widget',
      {
        id: widgetId,
      },
      { headers },
    ...
    // Verify DB deletion
    const dbWidgetsAfterDelete = await queryDb('SELECT * FROM chat_widget WHERE id = $1', [
      widgetId,
    ]);
