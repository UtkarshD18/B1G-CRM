    ...
    <div className="page-stack">
      <div className="page-header">
        <div>
          ...
          <h2>My Dashboard</h2>
          <p>View your assigned conversations, pending tasks, and performance metrics.</p>
        </div>
        <button className="primary-button" type="button" onClick={loadAgentWorkspace}>
          Refresh
        </button>
      </div>
      ...
      <div className="dashboard-grid">
        <DashboardCard
          title="Agent Name"
          value={agent?.name || '-'}
          detail={agent?.email || 'No profile loaded'}
        />
        <DashboardCard
          title="Assigned Chats"
          value={agent?.assignedChatsCount ?? chats.length}
          detail="Active conversation threads"
        />
        <DashboardCard
          title="Pending Tasks"
          value={agent?.pendingTasksCount ?? tasks.filter((t) => t.status !== 'COMPLETED').length}
          detail="Action items assigned to you"
        />
        <DashboardCard
          title="Resolved Chats"
          value={agent?.resolvedConversationsCount ?? 0}
          detail="Resolved ticket threads"
        />
        <DashboardCard
          title="Avg Response Time"
          value={agent?.averageResponseTime || '11 mins'}
          detail="SLA target within 15 mins"
        />
      </div>
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, rgba(25, 42, 86, 0.05) 0%, rgba(9, 132, 227, 0.05) 100%)',
        ...
        >
          <div>
            ...
            <p style={{ margin: '4px 0 0 0', fontSize: '14px', color: '#7f8c8d' }}>
              Real-time agent productivity index and resolution rates.
            </p>
          </div>
          <span
            style={{
                  ...
                  <tr key={chat.chat_id}>
                    ...
                    <td>
                      <button
                        ...
                        type="button"
                        onClick={() => updateChatStatus(chat.chat_id)}
                      >
                        Save
                      </button>
                    </td>
