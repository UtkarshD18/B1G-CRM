...
| Active JWT payload includes password hash | Login routes sign `{ password: storedHash }`. | Sensitive hash material is client-held. | Move to `uid`/`role` payload and DB lookup by `uid`; add expiry. |
| Active login tokens have no explicit expiry | Login routes pass `{}` to `sign()`. | Long-lived sessions. | Use `env.JWT_EXPIRY`. |
| Public install/update routes mutate files | `routes/web.js` exposes `install_app` and `update_app`; update only checks admin password in body. | Production takeover risk. | Restrict/remove or require admin auth plus deployment controls. |
...
| Debug routes remain public | `/api/inbox/`, `/api/qr/create`, `/api/qr/send`, `/api/web/`. | Noise/security surface. | Remove or restrict in production. |
