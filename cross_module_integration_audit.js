...
async function testFlowChatbotIntegration() {
  ...
  log(
    'FLOW→CHATBOT',
    'Get flows',
    flows.success && flows.data?.length > 0,
    `Found ${flows.data?.length || 0} flows`,
  );
  ...
  if (testBot) {
    ...
    log(
      'FLOW→CHATBOT',
      'Chatbot has flow_id',
      !!attachedFlow.flow_id,
      `flow_id: ${attachedFlow.flow_id}`,
    );
    log('FLOW→CHATBOT', 'Chatbot active', testBot.active === 1, `active: ${testBot.active}`);

    const flowDetail = await api(
      'POST',
      '/api/chat_flow/get_by_flow_id',
      { flowId: flow.flow_id },
      userToken,
    );
    log(
...
async function testChatbotDiagnostics() {
  ...
  log(
    ...
    'Get chatbots',
    chatbots.success,
    `Found ${chatbots.data?.length || 0} chatbots`,
  );
  ...
  log(
    ...
    'Get diagnostic logs',
    logs_res.success,
    `Found ${logs_res.data?.length || 0} logs`,
  );
...
async function testContactCampaignIntegration() {
  ...
  log(
    ...
    'Get phonebooks',
    phonebooks.success,
    `Found ${phonebooks.data?.length || 0} phonebooks`,
  );
  ...
  log(
    ...
    'Get contacts',
    contacts.success,
    `Found ${contacts.data?.length || 0} contacts`,
  );
  ...
  log(
    ...
    'Get campaigns',
    campaigns.success,
    `Found ${campaigns.data?.length || 0} campaigns`,
  );
  ...
  if (campaigns.data?.length > 0) {
    for (const camp of campaigns.data.slice(0, 3)) {
      try {
        ...
        log(
          'CONTACT→CAMPAIGN',
          `Campaign "${camp.title}" phonebook valid`,
          pbExists,
...
async function testAiProviderIntegration() {
  ...
  log(
    ...
    'Get AI providers',
    providers.success,
    `Found ${providers.data?.length || 0} providers`,
  );
  ...
  try {
    ...
    log(
      'AI→CHATBOT',
      'AI module loadable',
      typeof singleReplyAi === 'function',
      'singleReplyAi is a function',
    );
  } catch (err) {
...
async function testCrmLeadsIntegration() {
  ...
  log(
    'CRM→CONTACTS',
    'Lead in list after creation',
    !!testLead,
    testLead ? `ID: ${testLead.id}` : 'Not found',
  );
  ...
  if (testLead) {
    ...
    log(
      'CRM→CONTACTS',
      'Lead absent after deletion',
      deleted,
      deleted ? 'Cleanup verified' : 'Lead still present',
    );
  }
}
...
async function testWebsiteWidgetIntegration() {
  ...
  log(
    ...
    'Get websites',
    websites.success,
    `Found ${websites.data?.length || 0} websites`,
  );
}
...
async function testSupervisorIntegration() {
  ...
  log(
    ...
    'Get escalations',
    escalations.success,
    `Found ${escalations.data?.length || 0} escalations`,
  );
  ...
  if (agentMetrics.htmlResponse || !agentMetrics.success) {
    ...
    log(
      'SUPERVISOR→AGENT',
      'Agent metrics via KPIs',
      hasPerAgent,
      hasPerAgent
        ? `Per-agent metrics available (${kpis.data.averageResponseTimePerAgent.length} entries)`
        : 'No per-agent data',
    );
  } else {
    log(
      ...
      `Found ${agentMetrics.data?.length || 0} agent metrics`,
    );
  }
}
...
async function testWebhookInboxIntegration() {
  ...
  log(
    ...
    'Get webhook rules',
    rules.success,
    `Found ${rules.data?.length || 0} rules`,
  );
  ...
  try {
    ...
    log(
      'WEBHOOK→INBOX',
      'Webhook engine loadable',
      typeof processWebhookRules === 'function',
      'processWebhookRules is a function',
    );
  } catch (err) {
...
async function testChatbotModules() {
  ...
  try {
    ...
    log(
      'CHATBOT→INBOX',
      'metaChatbotInit loadable',
      typeof metaChatbotInit === 'function',
      'function loaded',
    );
  } catch (err) {
  ...
  try {
    ...
    log(
      'CHATBOT→INBOX',
      'recordChatbotLog loadable',
      typeof recordChatbotLog === 'function',
      'function loaded',
    );
  } catch (err) {
  ...
  try {
    ...
    log(
      'CHATBOT→INBOX',
      'destributeTaskFlow loadable',
      typeof destributeTaskFlow === 'function',
      'function loaded',
    );
  } catch (err) {
...
async function testFlowDeleteCascade() {
  ...
  log(
    'FLOW_DELETE→CHATBOT',
    'Flow delete cascades to chatbot',
    hasDeactivation,
...
async function testSlaEscalation() {
  ...
  log(
    'SLA→SUPERVISOR',
    'SLA checker exists',
    serverCode.includes('checkSlaEscalations'),
    'interval running',
  );
  log(
    'SLA→SUPERVISOR',
    'Escalation queue insert',
    serverCode.includes('INSERT INTO escalation_queue'),
    'auto-escalation',
  );
  ...
  log(
    'SLA→SUPERVISOR',
    'SLA timer on incoming msg',
    inboxCode.includes('sla_expires_at'),
    'timer updated',
  );
}
...
async function testDashboardCampaignIntegration() {
  ...
  log(
    ...
    summary.success,
    summary.data
      ? `Campaigns: ${summary.data.campaignStatus?.total || 0}, Delivery: ${summary.data.delivery?.total || 0}`
      : 'No data',
  );
}
...
async function main() {
  ...
  const fs = require('fs');
  fs.writeFileSync(
    __dirname + '/cross_module_audit_results.json',
    JSON.stringify(
      {
