...
(async () => {
  ...
  try {
    ...
    const agentLoginRes = await axios.post('http://localhost:3010/api/agent/login', {
      ...
      password: process.env.TEST_AGENT_PASSWORD || 'User@123'
    });
    ...
    const modifyPassRes = await axios.get('http://localhost:3010/api/user/modify_password?pass=User@123', {
    ...
    const modifyAdminPassRes = await axios.get('http://localhost:3010/api/admin/modify_password?pass=Admin@123', {
