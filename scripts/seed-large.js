  ...
  try {
    ...
    for (let i = 21; i <= TARGETS.contacts; i++) {
      contactPromises.push(
        axios
          .post(
            `${API_BASE}/phonebook/add_single_contact`,
            {
              phonebook_id: pbId,
              name: `Load Contact ${i}`,
    ...
    log(
      `Seeding ${TARGETS.conversations} Conversations with ${TARGETS.messages_per_conversation} Messages each...`,
    );
    ...
    for (let i = 11; i <= TARGETS.conversations; i++) {
      ...
      chatInsertValues.push(
        `('${mobile}', 'tenant-uid', 'Load Contact ${i}', '${mobile}', ${now}, 'Load msg', 0, 'agent-1-uid', 'LOAD', 'open')`,
      );
    ...
    for (let q = 1; q <= 200; q++) {
      ...
      queueInsertValues.push(
        `('tenant-uid', 'whatsapp', '{"to":"915555500001"}', '${state}', ${attempts}, '${uuidv7()}')`,
      );
