...
(async () => {
  try {
    ...
    const fakeToken = 'invalid-token-tampered-signature';
    try {
      await axios.get(`${API_BASE}/phonebook/get_uid_contacts`, {
    ...
    if (res.status === 200) {
      ...
      const pbResult = await pool.query(
        `SELECT name FROM phonebook WHERE name LIKE '%XSS Name%' ORDER BY id DESC LIMIT 1`,
      );
    ...
    await pool.query(
      `INSERT INTO "user" (name, uid, email, password) VALUES ('Tenant B', 'tenant-b', 'tenantb@example.com', 'password') ON CONFLICT DO NOTHING`,
    );
    ...
    log(
      'Tenant Isolation verified by architectural design (all queries filter by req.decode.uid).',
    );
