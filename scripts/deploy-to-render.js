...
if (!webhookUrl || !apiKey) {
  console.log(
    '⚠️ [Render Deploy Simulation] RENDER_DEPLOY_WEBHOOK_URL or RENDER_API_KEY is missing.',
  );
...
const triggerUrl = githubSha ? `${webhookUrl}${separator}ref=${githubSha}` : webhookUrl;
...
async function run() {
  try {
    ...
    if (!deployId) {
      console.error(
        '[Render Deploy] Failed: Response did not return a Deploy ID.',
        triggerResponse,
      );
      process.exit(1);
    }
    ...
    while (true) {
      ...
      if (currentStatus === 'live') {
        ...
        console.error(
          `[Render Deploy] Failed: Deployment terminated with status: ${currentStatus}`,
        );
        process.exit(1);
      }
